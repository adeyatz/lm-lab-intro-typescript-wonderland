...
import { House_Types, HOUSE_TYPES } from "../chapter_7/chapter_7_types";
...
export function parseHouseInput(input: string): House_Types | undefined {
  const chosenHouse = parseInt(input);

  // now we verify it's valid
  if (isNaN(chosenHouse)) {
    return undefined;
  }

  if (chosenHouse < 0 || chosenHouse > HOUSE_TYPES.length - 1) {
    return undefined;
  }

  // we know the input is valid so we can return a Hole
  return HOUSE_TYPES[chosenHouse];
}
